 ...
 */
public class ExpertQueueDetailAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ProblemJBException {
        ...
        String forward = request.getParameter("print")==null?"ExpertQueueDetail":"ExpertQueueDetail-p";
        ...
        if ((request.getMethod()).equals("GET") || (request.getMethod()).equals("POST")) {
            ...
            String QBEOrder = null;
            String QBEExtendedSearch = null;
            
            if (closed.equals("QBE")) {
                QBEDescrizione = request.getParameter("Descrizione");
                QBEExtendedSearch = request.getParameter("extendedSearch");
                if (QBEDescrizione != null) {
                    se.setAttribute("it.cnr.helpdesk.QBEDescrizione", QBEDescrizione);
                    se.setAttribute("it.cnr.helpdesk.QBEExtendedSearch", QBEExtendedSearch);
                } else {
                    QBEDescrizione = (String) se.getAttribute("it.cnr.helpdesk.QBEDescrizione");
                    QBEExtendedSearch = (String) se.getAttribute("it.cnr.helpdesk.QBEExtendedSearch");
                }
                ...
                QBEOrder = request.getParameter("ordinamento");

                if (QBEOrder != null) {
                    se.setAttribute("it.cnr.helpdesk.QBEOrder", QBEOrder);
                } else {
                    QBEOrder = (String) se.getAttribute("it.cnr.helpdesk.QBEOrder");
                }
            }
			...
			try {
				pbp = new PageByPageIteratorImpl(pageNumber, Integer.parseInt(Settings.getProperty("it.oil.itemsInPage",se.getAttribute("it.cnr.helpdesk.instance").toString())), "ExpertQueueDetail.do?" + queryString, Integer.parseInt(Settings.getProperty("it.oil.pagesInNavigator",se.getAttribute("it.cnr.helpdesk.instance").toString())));
			} catch (Exception e) {
            ...
            if (closed.equals("NO") || closed.equals("YES") || closed.equals("VER") || closed.equals("STO")) {
                ...
                if (closed.equals("NO"))
                    p = problem.getUnclosedExpertProblemQueue(pbp, pvo, instance);
                else if (closed.equals("YES"))
                    p = problem.getClosedExpertProblemQueue(pbp, pvo, instance);
                else if (closed.equals("VER"))
                    p = problem.getVerifiedExpertProblemQueue(pbp, pvo, instance);
                else if (closed.equals("STO"))
                    p = problem.getClosedAndVerifiedProblemQueue(pbp, pvo, instance);
                ...
                pvo.setExtendedSearch(QBEExtendedSearch!=null);
