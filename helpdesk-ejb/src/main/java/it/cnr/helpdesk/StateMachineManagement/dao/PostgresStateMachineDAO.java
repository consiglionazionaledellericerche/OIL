...
public class PostgresStateMachineDAO extends HelpDeskDAO implements
		StateMachineDAO {
	...
	public void createConnection(String instance) throws StateMachineDAOException {
		try {
			...
			throw new StateMachineDAOException(helpdeskdaoexception
					.getMessage(), helpdeskdaoexception.getUserMessage());
		}
	}
	...
	/*
	 * <p>Legge le transizioni dal db, associandole ai profili autorizzati ad
	 * eseguirle.</p> @param @return Restituisce un ArrayList di oggetti di
	 * tipo it.cnr.helpdesk.StateMachineManagement.valueobjects.Transition
	 * 
	 */
	...
	public HashMap readTransitions() throws StateMachineDAOException {
		...
		String s = "Select p.id_profilo, p.nome, t.id, t.origine, s1.descrizione, t.destinazione, s2.descrizione "+
			"from transition t, stato s1, stato s2, profilo p, _authorization a "+
	...
	/*
	 * <p>Legge le precondizioni associate ad una transizione, ovvero le
	 * condizioni che devono essere verificate prima di effettuare la
	 * transizioni.</p> @param int transition identificativo della transizione.
	 * @return Restituisce un ArrayList di condizioni associate alla transizione
	 * specificata.
	 */
	public ArrayList readPreConditions(int transition)
			throws StateMachineDAOException {
		String s = "Select c.condition_class "
				+ "from transition t, pre_conditions pc, condition c "
				+ "where t.id=pc.transition " + "and pc.condition=c.id "
				+ "and t.id=" + transition;
		ArrayList arraylist = null;
		try {
			ResultSet rs = execQuery(s);
			...
			while (rs.next()) {
				...
				Condition condition;
				try {
					condition = (Condition) Class.forName(conditionName)
							.newInstance();
				} catch (InstantiationException e) {
					...
					e.printStackTrace();
					throw new StateMachineDAOException(e.getMessage(),
							"Impossibile accedere alla definizione della classe "
									+ conditionName + ".");
				} catch (ClassNotFoundException e) {
					
					e.printStackTrace();
					throw new StateMachineDAOException(e.getMessage(),
							"Impossibile trovare la definizione della classe "
									+ conditionName + ".");
				}
	...
	/*
	 * <p>Legge le postcondizioni associate ad una transizione, ovvero le
	 * condizioni che devono essere verificate dopo aver effettuato la
	 * transizioni.</p> @param int transition identificativo della transizione.
	 * @return Restituisce un ArrayList di condizioni associate alla transizione
	 * specificata.
	 */
	public ArrayList readPostConditions(int transition)
			throws StateMachineDAOException {
		String s = "Select c.condition_class "
