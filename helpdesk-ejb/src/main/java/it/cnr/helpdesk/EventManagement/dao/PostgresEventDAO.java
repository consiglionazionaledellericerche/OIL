 ...
 */
public class PostgresEventDAO extends HelpDeskDAO implements EventDAO {
	...
	public String register(EventValueObject evo) throws EventDAOException {
		//String s = "select TO_CHAR(now(),'DD-MM-YYYY HH24:MI:SS')";
		...
		try {
			...
			s1 = "insert into evento(SEGNALAZIONE, DATETIME, TIPO_EVENTO, STATO, CATEGORIA, NOTA, ORIGINATORE) " +
			  "values(" + evo.getIdSegnalazione() + ", to_timestamp('" + ora + "','DD-MM-YYYY HH24:MI:SS') ,"+evo.getEventType()+","+evo.getState()+"," + evo.getCategory() + ",'" + evo.getNote2SQL() + "','" + evo.getOriginatoreEvento() + "')";
			log(this, s1);
	        execUpdQuery(s1);
	        ...
	        throw new EventDAOException(sqlexception1.getMessage(), "Si \350 verificato un errore durante il tracciamento dell'evento. Contattare l'assistenza.");
	    }
        return ora;
	}
...
public EventValueObject getProblemEventDetail(int segnalazione, String dataEvento) throws EventDAOException {
    ...
    String s = "select e.SEGNALAZIONE,TO_CHAR(e.DATETIME,'DD-MM-YYYY HH24:MI:SS'),e.TIPO_EVENTO, te.NOME_TIPO_EVENTO,e.STATO, s.DESCRIZIONE, e.CATEGORIA,c.NOME, e.NOTA,e.ORIGINATORE,coalesce(u.NOME,'-'),coalesce(u.COGNOME,'-') " +
    		   "from (evento e LEFT JOIN stato s ON e.STATO=s.ID_STATO) LEFT JOIN categoria c ON e.CATEGORIA=c.ID_CATEGORIA, tipo_evento te, utente u " +
    		   "where e.SEGNALAZIONE=" + segnalazione + " and e.TIPO_EVENTO=te.ID_TIPOEVENTO and e.ORIGINATORE=u.USERID and e.DATETIME=to_timestamp('" + dataEvento + "','DD-MM-YYYY HH24:MI:SS')";
    try
    {
        log(this, s);
...
public Collection getProblemEvents(long segnalazione, boolean filter) throws EventDAOException {
	...
	String s = "select e.SEGNALAZIONE,TO_CHAR(e.DATETIME,'DD-MM-YYYY HH24:MI:SS'),e.TIPO_EVENTO, te.NOME_TIPO_EVENTO,e.STATO, s.DESCRIZIONE, e.CATEGORIA,c.NOME, e.NOTA,e.ORIGINATORE,coalesce(u.NOME,'-'),coalesce(u.COGNOME,'-'), e.ALTERABLE " +
			   "from (evento e LEFT JOIN stato s ON e.STATO=s.ID_STATO) LEFT JOIN categoria c ON e.CATEGORIA=c.ID_CATEGORIA , tipo_evento te, utente u " +
			   "where e.SEGNALAZIONE=" + segnalazione + " and e.TIPO_EVENTO=te.ID_TIPOEVENTO and e.ORIGINATORE=u.USERID " + filtro + " order by datetime";
	try
	{
		log(this, s);
		EventValueObject eventvalueobject;
