 ...
 */
public class OracleMessageDAO extends HelpDeskDAO implements MessageDAO {
   ...
   /**
     * Restituisce tutti i messaggi all'utenza filtrando opzionalmente,
     * in base al parametro filter, quelli con flag enabled=y
     * @param filter abilita o meno il filtro sui messaggi disabilitati
	...
	public int executeInsert(MessageValueObject mvo) throws MessageDAOException {
		...
		int i = 0;
		try {
			s = "select seq_messaggi.nextval from dual";
			ResultSet resultset = execQuery(s);
			if (resultset.next())
				i = resultset.getInt(1);
			s = "insert into MESSAGGIO(id_msg, TESTO, data, enabled, originatore) values (" + i + ", '" + mvo.getTesto().replaceAll("'", "''") + "',sysdate, 'y', '" + mvo.getOriginatore() + "')";
			log(this, s);
			execUpdQuery(s);
	...
	public void executeUpdate(MessageValueObject mvo) throws MessageDAOException {
		String query = "UPDATE MESSAGGIO SET TESTO = '" + mvo.getTesto().replaceAll("'","''") + "', data = sysdate WHERE ID_MSG = " + mvo.getId_msg();
		try {
        	execUpdQuery(query);
            System.out.println(query);
        } catch(SQLException sqlexception) {
