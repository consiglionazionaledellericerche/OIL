 ...
 */
public class OracleAttachmentDAO extends HelpDeskDAO implements AttachmentDAO{
	...
	public void executeInsert() throws AttachmentDAOException {
		String s = "select seq_attachments.nextval from dual";
		Object obj = null;
		int i = 0;
		try {
        	...
        	s = "insert into attachments(ID,ID_SEGNALAZIONE, ATTACHMENT, DESCRIZIONE, NOME_FILE) values(" + i + "," + id_segnalazione + ", empty_blob(),'"+descrizione+"','"+nomeFile+"')";
        	log(this, s);
        	execUpdQuery(s);
        	s="select ATTACHMENT  from  ATTACHMENTS where ID="+i;
        	resultset=execQuery(s);
        	resultset.next();
        	BLOB blob = (BLOB) resultset.getBlob("ATTACHMENT");
        	java.io.InputStream in = new java.io.BufferedInputStream(new FileInputStream(this.attachment));
