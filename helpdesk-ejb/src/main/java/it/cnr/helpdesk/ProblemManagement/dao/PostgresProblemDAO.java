...
public class PostgresProblemDAO extends HelpDeskDAO implements ProblemDAO {
	...
	public ProblemValueObject getProblemDetail() throws ProblemDAOException {
		...
		/*
		String s = "select segnalazione.ID_SEGNALAZIONE,segnalazione.TITOLO,segnalazione.DESCRIZIONE,segnalazione.CATEGORIA,categoria.NOME, "
				+ "       segnalazione.STATO,stato.DESCRIZIONE,segnalazione.ORIGINATORE,coalesce(ori.NOME,'-'),coalesce(ori.COGNOME,'-'), "
				+ "       segnalazione.ESPERTO,coalesce(esp.NOME,'-'),coalesce(esp.COGNOME,'-'),segnalazione.FLAGFAQ "
				+ "from   utente esp RIGHT JOIN segnalazione on esp.USERID = segnalazione.ESPERTO, categoria,stato,utente ori  "
				+ "where  segnalazione.CATEGORIA=categoria.ID_CATEGORIA and "
				+ "       segnalazione.STATO=stato.ID_STATO and "
				+ "	     segnalazione.ORIGINATORE=ori.USERID and "
				+ "       segnalazione.ID_SEGNALAZIONE=" + this.idSegnalazione;
		*/
		String s = "select segnalazione.ID_SEGNALAZIONE,segnalazione.TITOLO,segnalazione.DESCRIZIONE,segnalazione.CATEGORIA,categoria.NOME, segnalazione.STATO,stato.DESCRIZIONE,segnalazione.ORIGINATORE,coalesce(ori.NOME,'-'),coalesce(ori.COGNOME,'-'), segnalazione.ESPERTO,coalesce(esp.NOME,'-'),coalesce(esp.COGNOME,'-'),segnalazione.FLAGFAQ,segnalazione.PRIORITA, priorita.DESCRIZIONE, segnalazione.VALIDATORE,coalesce(val.NOME,'-'),coalesce(val.COGNOME,'-') "
			+ "from   utente val RIGHT JOIN (utente esp RIGHT JOIN segnalazione on esp.USERID = segnalazione.ESPERTO) on val.USERID = segnalazione.VALIDATORE , categoria,stato,utente ori, priorita  "
			+ "where  segnalazione.CATEGORIA=categoria.ID_CATEGORIA and "
			+ "       segnalazione.STATO=stato.ID_STATO and "
			+ "	  segnalazione.ORIGINATORE=ori.USERID and "
			+ "	  segnalazione.PRIORITA=priorita.LIVELLO and "
			+ "       segnalazione.ID_SEGNALAZIONE=" + this.idSegnalazione;
		...
		try {
			...
			if (resultset.next())
				problemvalueobject = new ProblemValueObject(resultset
						.getLong(1), resultset.getString(2), resultset
						.getString(3), resultset.getInt(4), resultset
						.getString(5), resultset.getInt(6), resultset
						.getString(7), resultset.getString(8), resultset
						.getString(9)
						+ " " + resultset.getString(10), resultset
						.getString(11), resultset.getString(12) + " "
						+ resultset.getString(13), resultset.getInt(14),resultset.getInt(15), resultset.getString(16),resultset.getString(17),resultset.getString(18)+" "+resultset.getString(19));
			...
			throw new ProblemDAOException(
					...
					"Si \350 verificato un errore durante il recupero delle informazioni di dettaglio relative alla segnalazione.");
		}
	...
	public PageByPageIterator getExpertProblemQueue(PageByPageIterator pagebypageiterator, ProblemValueObject problemvalueobject) throws ProblemDAOException {
		...
		if(problemvalueobject.getProfiloUtente()==4){// UTENTE VALIDATORE
			...
			// vale solo per il validatore perch� per l'expert dalla Action client non viene settato 
			...
			if(codStatoSegnalazione == 4){//recupero segnalazioni chiuse solo del VALIDATORE
				...
				"segnalazione.TITOLO," +
				"stato.DESCRIZIONE," +
				"coalesce(originatore.NOME,'-')," +
				"coalesce(originatore.COGNOME,'-')," +
				"originatore.USERID," +
				"coalesce(esperto.NOME,'-')," +
				"coalesce(esperto.COGNOME,'-')," +
				...
				"segnalazione.TITOLO," +
				"stato.DESCRIZIONE," +
				"coalesce(utente.NOME,'-')," +
				"coalesce(utente.COGNOME,'-')," +
				"utente.USERID," +
				"' - '," +
				"' - '," +
				"TO_CHAR(evento.DATETIME,'DD-MM-YYYY HH24:MI:SS')," +
			...
			if(problemvalueobject.isPerCategory()) {
				if (problemvalueobject.isOwnerShip())
					s = "select segnalazione.ID_SEGNALAZIONE," +
						...
						"categoria.NOME, " +
						"strutture.evidenza " +
						"from utente LEFT JOIN strutture ON utente.struttura = strutture.codice_struttura,stato,segnalazione,evento, categoria " +
						"where segnalazione.STATO=stato.ID_STATO and " +
						...
						"categoria.NOME, " +
						"strutture.evidenza " +
						"from segnalazione,stato,utente originatore LEFT JOIN strutture ON originatore.struttura = strutture.codice_struttura,evento, utente esperto, categoria " +
						"where segnalazione.STATO=stato.ID_STATO and " +
					...
					s = "select segnalazione.ID_SEGNALAZIONE," +
							...
							"coalesce(originatore.NOME,'-')," +
							"coalesce(originatore.COGNOME,'-')," +
							"originatore.USERID,coalesce(esperto.NOME,'-')," +
							"coalesce(esperto.COGNOME,'-')," +
							"TO_CHAR(evento.DATETIME,'DD-MM-YYYY HH24:MI:SS')," +
							"segnalazione.STATO," +
							"segnalazione.ESPERTO," +
				...
				s = "select segnalazione.ID_SEGNALAZIONE," +
						...
						"categoria.NOME, " +
						"strutture.evidenza " +
						"from stato,utente originatore LEFT JOIN strutture ON originatore.struttura = strutture.codice_struttura,evento, utente esperto right outer join segnalazione on (esperto.USERID=segnalazione.esperto), categoria " +
						"where segnalazione.STATO=stato.ID_STATO and " +
						"segnalazione.originatore=originatore.USERID and " +
						"segnalazione.ID_SEGNALAZIONE=evento.SEGNALAZIONE and " +
						"evento.TIPO_EVENTO in (0,7) and " +
			...
			try {
				...
				for (ResultSet resultset = execQuery(s); resultset.next();) {
					i = resultset.getRow();
					if (i >= pagebypageiterator.getStart()
							&& i <= (pagebypageiterator.getStart() + pagebypageiterator
									.getSize()) - 1) {
						ProblemListItemValueObject problemlistitemvalueobject = new ProblemListItemValueObject(
								resultset.getLong(1), resultset.getString(2),
								resultset.getString(3), resultset.getString(4)
								+ " " + resultset.getString(5), resultset
								.getString(6), resultset.getString(7) + " "
								+ resultset.getString(8), resultset
								.getString(9), resultset.getInt(10),
    ...
    public PageByPageIterator getTeamProblemQueue(PageByPageIterator pagebypageiterator, ProblemValueObject problemvalueobject)  throws ProblemDAOException {
    	String s = null;
    	s = "select S.ID_SEGNALAZIONE,  S.TITOLO,  ST.DESCRIZIONE, coalesce(UO.NOME,'-'),  coalesce(UO.COGNOME,'-'),  " +
		"UO.USERID, coalesce(ES.NOME,'-'),  coalesce(ES.COGNOME,'-'), TO_CHAR(E.DATETIME,'DD-MM-YYYY HH24:MI:SS'), " +
	...
	public PageByPageIterator getUserProblemQueue(
			PageByPageIterator pagebypageiterator,
			ProblemValueObject problemvalueobject) throws ProblemDAOException {
		String s = "select segnalazione.ID_SEGNALAZIONE,segnalazione.TITOLO,stato.DESCRIZIONE,NULL,NULL,NULL,coalesce(esperto.NOME,'-'),coalesce(esperto.COGNOME,'-'),TO_CHAR(evento.DATETIME,'DD-MM-YYYY HH24:MI:SS'),segnalazione.STATO,segnalazione.ESPERTO,categoria.NOME from segnalazione left outer join utente esperto on esperto.USERID=segnalazione.esperto,stato,evento, categoria where segnalazione.STATO=stato.ID_STATO and       segnalazione.originatore='"
				+ problemvalueobject.getOriginatore()
				+ "' and "
				+ "      segnalazione.ID_SEGNALAZIONE=evento.SEGNALAZIONE and "
				+ "      segnalazione.CATEGORIA=categoria.ID_CATEGORIA and "
				+ "      evento.TIPO_EVENTO in (0,7) "
		...
		try {
			...
			throw new ProblemDAOException(
					sqlexception.getMessage(),
					"Si \350 verificato un errore durante il recupero dellalista delle segnalazioni.");
		}
		pagebypageiterator.setTotalCount(i);
		pagebypageiterator.setCollection(arraylist);
		return pagebypageiterator;
	...
	public Collection getProblemCountPerCategory(
			ProblemCountValueObject problemcountvalueobject)
		...
		if(problemcountvalueobject.getProfiloUtente()==4) { // se utente validatore
		...
		else{ // altri tipi di utente. La QUERY che segue � relativa ad UTENTE ESPERTO
			if (problemcountvalueobject.isOwnerShip())
				...
				s = "Select Segnalazione.Categoria,Categoria.nome,count(*)  from Categoria , Segnalazione ,Esperto_Categoria   where  Segnalazione.Categoria=Esperto_Categoria.Categoria "
						+ problemcountvalueobject.getSearchingCollection()
								.toSQLString()
						+ " and Esperto_Categoria.Esperto='"
						+ problemcountvalueobject.getExpertLogin()
						+ "' "
						+ " and Categoria.id_Categoria=Esperto_Categoria.Categoria "
						+ " group by Segnalazione.Categoria, Categoria.nome";
		}
		try {
			...
			for (ResultSet resultset = execQuery(s); resultset.next(); arraylist
					.add(problemcountvalueobject1))
				problemcountvalueobject1 = new ProblemCountValueObject(
			...
			throw new ProblemDAOException(sqlexception.getMessage(),
					"Si \350 verificato un errore durante il conteggio dei problemi per categoria.");
		}
		return arraylist;
	}
        ...
        throws ProblemDAOException
    {
        ...
        try
        {
            ...
            String s2 = "insert into evento(SEGNALAZIONE, DATETIME, TIPO_EVENTO, STATO, CATEGORIA, NOTA,ORIGINATORE) values(" + idSegnalazione + ",now(),3," + j + "," + i + ",'" + formatString(s) + "','" + originatoreEvento + "')";
            log(this, s2);
            execUpdQuery(s2);
        }
        catch(SQLException sqlexception)
    ...
    {
        ArrayList arraylist = new ArrayList();
        String s = "select evento.SEGNALAZIONE,TO_CHAR(evento.DATETIME,'DD-MM-YYYY HH24:MI:SS'),evento.TIPO_EVENTO, tipo_evento.NOME_TIPO_EVENTO,evento.STATO, stato.DESCRIZIONE, evento.CATEGORIA,categoria.NOME, evento.NOTA,evento.ORIGINATORE,coalesce(utente.NOME,'-'),coalesce(utente.COGNOME,'-') from (evento LEFT JOIN stato ON evento.STATO=stato.ID_STATO) LEFT JOIN categoria ON evento.CATEGORIA=categoria.ID_CATEGORIA , tipo_evento , utente where evento.SEGNALAZIONE=" + idSegnalazione + " and evento.TIPO_EVENTO=tipo_evento.ID_TIPOEVENTO and evento.ORIGINATORE=utente.USERID order by datetime";
        try
        {
            log(this, s);
        ...
        throws ProblemDAOException
    {
        ...
        String s = "select evento.SEGNALAZIONE,TO_CHAR(evento.DATETIME,'DD-MM-YYYY HH24:MI:SS'),evento.TIPO_EVENTO, tipo_evento.NOME_TIPO_EVENTO,evento.STATO, stato.DESCRIZIONE, evento.CATEGORIA,categoria.NOME, evento.NOTA,evento.ORIGINATORE,coalesce(utente.NOME,'-'),coalesce(utente.COGNOME,'-') from (evento LEFT JOIN stato ON evento.STATO=stato.ID_STATO) LEFT JOIN categoria ON evento.CATEGORIA=categoria.ID_CATEGORIA, tipo_evento, utente where evento.SEGNALAZIONE=" + idSegnalazione + " and evento.TIPO_EVENTO=tipo_evento.ID_TIPOEVENTO and evento.ORIGINATORE=utente.USERID and evento.DATETIME=to_timestamp('" + dataEvento + "','DD-MM-YYYY HH24:MI:SS')";
        try
        {
            log(this, s);
            ResultSet resultset = execQuery(s);
	...
	/* (non-Javadoc)
	 * @see it.cnr.helpdesk.dao.ProblemDAO#changeState(long, int)
	 */
	...
	public long executeInsert(ProblemValueObject pvo) throws ProblemDAOException {
		String s = "select nextval('seq_segnalazione')";
        int i = 0;
        try {
            ResultSet resultset = execQuery(s);
            if(resultset.next())
	...
	/* (non-Javadoc)
	 * @see it.cnr.helpdesk.dao.ProblemDAO#assignProblemToExpert(it.cnr.helpdesk.ProblemManagement.valueobjects.EventValueObject)
	 */
	...
	public void releaseProblemByExpert(EventValueObject evo) throws ProblemDAOException {
	    String s1 = "update segnalazione set ESPERTO=null where ID_SEGNALAZIONE=" + evo.getIdSegnalazione();
	    try {
	        log(this, s1);
	        execUpdQuery(s1);
	    } catch(SQLException sqlexception) {
	...
	/* (non-Javadoc)
	 * @see it.cnr.helpdesk.dao.ProblemDAO#reAssignProblemToExpert(java.lang.String)
	 */
    public void reAssignProblemToExpert(EventValueObject evo) throws ProblemDAOException {
    	...
    	try
		{
    		...
    		log(this, s1);
    		execUpdQuery(s1);
    		s1 = "insert into evento(SEGNALAZIONE, DATETIME, TIPO_EVENTO, STATO, CATEGORIA, NOTA, ORIGINATORE) values(" + evo.getIdSegnalazione() + ",now(),4," + j + "," + i + ",'" + evo.getNote2SQL() + "','" + evo.getOriginatoreEvento() + "')";
    		log(this, s1);
    		execUpdQuery(s1);
		}
    	catch(SQLException sqlexception)
